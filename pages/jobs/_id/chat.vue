<template>
  <div class="chat-messages">
    <div class="flex flex-col h-full bg-red">
      <div class="flex pt-12">
        <div class="flex-1 justify-center">
          <button class="mr-auto p-2 -ml-2 focus:outline-none" @click.prevent="$router.back()">
            <i class="far fa-chevron-left" />
          </button>
        </div>

        <div class="flex-3 justify-center text-center pb-2">
          <p>{{ offer.title }}</p>
          <p class="text-xs text-gray-400 font-semibold">Mo - Mi, 12:00 - 18:00</p>
        </div>

        <div class="flex-1 justify-center"><span class="ml-auto" /></div>
      </div>

      <div class="bg-gray-100 -mx-4 px-4 py-3">
        <employer-banner :offer="offer" />
      </div>

      <div class="message-list flex-1 overflow-y-scroll -mx-4 px-4" ref="messageList">
        <div class="flex items-center justify-center h-full" v-if="!messages.length">
          <div class="-mt-24">
            <svg class="mx-auto" width="60" height="61" viewBox="0 0 60 61" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
              <path d="M0 60.2H60V0.2H0V60.2Z" fill="url(#pattern0)"/>
              <defs>
                <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                  <use xlink:href="#image0" transform="scale(0.015625)"/>
                </pattern>
                <image id="image0" width="64" height="64" xlink:href="data:image/png;base64,"/>
              </defs>
            </svg>

            <p class="px-14 mt-2 text-center leading-tight text-xs text-gray-400">
              Füge weitere Informationen hinzu oder stelle dich vor.
            </p>
          </div>
        </div>

        <div class="pt-4 flex flex-col" v-else>
          <div v-for="message in messages" :key="message.id"
            class="message mb-4 py-2 px-4 bg-catskills inline-block"
            :class="{ me: message.authorId == user.uid, sequential: message.sequential, final: message.final }"
          >
            {{ message.message }}
          </div>
        </div>
      </div>

      <form @submit.prevent="submit" class="input-window bg-gray-100 p-4 flex -mx-4 -mb-px">
        <input type="text" v-model="chatMessage" class="focus:outline-none bg-catskills rounded-lg p-2 flex-1">
        <button class="p-2 ml-2 -mr-2 text-salmon focus:outline-none" type="submit"><i class="far fa-paper-plane" /></button>  
      </form>
    </div>
  </div>
</template>

<script lang="ts">
import Vue from 'vue'
import { IConversation, IChatMessage } from '~/types'
import EmployerBanner from '~/components/Offers/EmployerBanner.vue'

type PageData = {
  conversation?: IConversation,
  chatMessage: string,
  refreshInterval: number | null,
}

type EnhancedMessage = IChatMessage & {
  sequential: boolean,
  final: boolean,
}

export default Vue.extend({
  components: {
    EmployerBanner,
  },

  async asyncData ({ params, $axios }) {
    const { data: conversation } = await $axios.get(`chat/${params.id}`)
    const { data: offer } = await $axios.get(`jobOffer/${params.id}`)

    return { offer, conversation }
  },

  data (): PageData {
    return {
      chatMessage: '',
      refreshInterval: null as number | null,
    }
  },

  computed: {
    user () {
      return this.$accessor.user.user
    },

    messages (): EnhancedMessage[] {
      const messages = this.conversation!.messages

      return messages.map((message, index) => {
        const lastMessage = messages[index - 1]
        const nextMessage = messages[index + 1]
        let sequential = false
        let final = false

        if (lastMessage && lastMessage.authorId == message.authorId) sequential = true
        if (!nextMessage || nextMessage.authorId != message.authorId) final = true

        return {
          ...message,
          sequential,
          final,
        }
      })
    },
  },

  mounted () {
    // Dirty polling for now
    this.refreshInterval = window.setInterval(() => this.refresh(), 1000)
    this.scrollToBottom()
  },

  beforeDestroy () {
    window.clearInterval(this.refreshInterval!)
  },

  methods: {
    async submit () {
      if (this.chatMessage.length == 0) return

      const { data: msg } = await this.$axios.post(`chat/${this.conversation?.jobOfferId}/send`, { message: this.chatMessage })

      this.conversation!.messages.push(msg)
      this.chatMessage = ''
      this.$nextTick(() => this.scrollToBottom())
    },

    async refresh () {
      const { data: conversation } = await this.$axios.get(`chat/${this.conversation!.jobOfferId}`)

      const newMessages = conversation.messages.length != this.conversation!.messages.length
      if (!newMessages) return

      this.conversation = conversation
      this.$nextTick(() => this.scrollToBottom())
    },

    scrollToBottom () {
      const messageList = this.$refs.messageList as Element
      messageList.scrollTop = messageList.scrollHeight
    },

    isMessageAuthor (msg : any) {
      if (this.conversation) {
        return msg.authorId === this.conversation.workerId
      }
      return false
    }
  },
})
</script>

<style lang="scss">
.chat-messages {
  height: calc(100vh - 70px);

  .message-list::-webkit-scrollbar {
    display: none;
  }

  .message {
    position: relative;
    border-radius: 16px;
    max-width: 80%;

    &.me {
      @apply bg-petrol text-white ml-auto;

      &.final::after {
        transform: scaleX(-1);
        border-color: #13314D transparent transparent transparent;
        left: unset;
        right: 0;
      }
    }

    &.sequential {
      @apply -mt-3;
    }

    &.final::after {
      content: "";
      position: absolute;
      bottom: -2px;
      left: 0;
      width: 0;
      height: 0;
      border-style: solid;
      border-width: 17px 20px 0 0;
      border-color: #E2E9F0 transparent transparent transparent;
    }
  }

  .employer-banner {
    .name {
      @apply text-sm font-bold;
    }

    .category {
      @apply text-petrol bg-catskills font-semibold;
    }
  }
}
</style>
